...
{
	//Create an <a> element for them
	var new_contact = document.createElement("a");
	new_contact.innerHTML = name;
	new_contact.addEventListener('click',call_new_chat_session,false);
	document.getElementById("gui_contact_list").appendChild(new_contact);
}
...
function add_potential_buddy(keyfingerprint,human_name)
{
	...
	document.getElementById("potential_buddies_list").appendChild(new_contact);
}
...
function new_chat_session(user)
{
	...
	var html_obj_1 = document.createElement("div");
	html_obj_1.className = "opened tunggal";
	var html_obj_2 = document.createElement("div");
	html_obj_2.className = "foldr small";
	var html_obj_3 = document.createElement("div");
	html_obj_3.className = "chat-win";
	var html_obj_4 = document.createElement("textarea");
	html_obj_4.className = "chatbox";
	var html_send_button = document.createElement("img");
	html_send_button.src = "icons/32x32mail-send.png";
	html_send_button.className = "sendbutton";
	
	//Define the send action
	//html_send_button.onclick = function()
	//{
	//	//DEBUG
	//	console.log("You pressed send");
	//	//Show the user the message which they have sent
...
function send_message()
{
	...
	local_conversation_obj.innerHTML += "<p style='color: green; text-align: left; margin-left: 15px'> You> " + local_text_entry_box_obj.value + "</p>";
	...
	// Encrypt the message
	var rx_pub_key = LOCAL_KEYSERVER[KEYRING[local_rx_obj.innerHTML]].keys;
	...
	text_message_pkt["payload"] = openpgp.signAndEncryptMessage(rx_pub_key,privKey,local_text_entry_box_obj.value);
	...
	text_message_pkt["dst"] = KEYRING[local_rx_obj.innerHTML];
	...
	mh_text_pkt["dst"] = KEYRING[local_rx_obj.innerHTML];
	...
	msg_path = get_path_to_node(KEYRING[local_rx_obj.innerHTML], s.graph.edges());
...
function add_buddy(k, human_name)
{
	...
	alertify.prompt("<table class='add_buddy_table'><tr><td style='vertical-align: top;'><img src='icons/64x64dialog-warning.png'></td><td><p>Verify the Primary Key Fingerprint to ensure that you are securely communicating.</p></br></br></br></br></td></tr><tr><td><p>Name: </p><input id='new_buddy_name' value="+human_name+"></input><p>Primary Key Fingerprint: </p><input id='new_buddy_pgp' value=" + k + "></input></td><td class='qr_camera'><img src='icons/128x128camera-web.png' /></td></tr></table><br/><br/>",
	function (e,str)
	{
		if (e)
		{
